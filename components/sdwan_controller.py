...
def run_controller_automation(
    ...
    out.header("Automation: CONTROLLER (vSmart)", f"Target: {config.mgmt_ip}")
...
def run_certificate_automation(net_connect, config: settings.ControllerConfig) -> bool:
    ...
    out.spinner_wait(
        f"Waiting Controller to be added and CSR to be generated ({settings.waits.csr_generation} seconds)...",
        settings.waits.csr_generation,
    )
